...
if __name__ == '__main__':
    ...
    with UsbKeyEcho(port, [p.encode() for p in sys.argv[1:]]) as ad:
        ad.reset()
        try:
            while True:
                ad.communicate()
        except KeyboardInterrupt:
            print('%u messages received, %u parse errors' % (
                ad.msg_cnt, ad.parse_errors
            ))
