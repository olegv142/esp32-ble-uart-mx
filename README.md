...
Using adapter with dual peripheral / central roles simultaneously makes the probability of data loss even higher. Possibly it is expected behavior. The central device is expected to schedule radio receive / transmit intervals for itself and for connected peripheral. So working with the same radio in two roles simultaneously is inherently problematic. Using multiple connections to peripheral devices also introduces some issues. There are two methods the central may use to write data to peripheral device. The fast method is writing without response. Writing with response is several times slower. It turns out that using writing without response results in sporadic connections dropping in case there are more then one connection to peripheral. The adapter may be configured in compile time to either use fast writing or not. If you don't need to connect to more than one device at a time than you can enable fast writing by defining FAST_WRITES in configuration file. The default setting is not using fast write method.
