...
Using adapter with dual peripheral / central roles simultaneously makes the probability of data loss even higher. Probably it is expected behavior. The central device is expected to schedule radio receive / transmit intervals for itself and for connected peripheral. So working with the same radio in two roles simultaneously is inherently problematic. Using multiple connections to peripheral devices also introduces some issues. Data receiving from multiple peripherals works reasonably well. But sending data to peripheral becomes very unreliable once second connection is open even if its completely idle. The only working way of transmitting data to multiple peripherals is sending one small (up to 240 bytes with extended frames enabled) message at a time. This obviously looks like a bug in the BLE stack implementation.

Another issue observed in tests is the possibility of deadlock while using CTS/RTS flow control in both directions. Its possible that both adapter and the host are trying to write to the serial channel while their receiving buffers are full. In such case both sides can't make progress. The adapter is able to recover from such freeze due to watchdog which reset it after 20 seconds of main loop inactivity. Another possible solution to this problem is to check if write to serial port would block before writing. Unfortunately, the ESP32 serial port does not provide correct information about the amount of available space in the write buffer. One can still implement separate serial port buffer. But this way we hit another serial port implementation flaw. When writes to serial port are blocked due to transmit fifo full the reads are blocked too for some unknown reason. So for now its not feasible to fix. Yet both CTS/RTS are rarely used, in most cases hardware flow control is not used at all.
